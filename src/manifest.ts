...
export const OPERATIONS: readonly Operation[] = [
  ...
  },
  {
    ...
    "responseSummary": "{ session_id, held: boolean, state: 'clear'|'grader_fooled'|'gamed'|'degrading', reason: string|null, pinned_step: string|null (the PIN: the earliest checkpoint that cannot be separated from the best one, with the baseline as a candidate — NOT the highest anchor read), improvement: { anchor, reward } paired difference vs the run's baseline with an anytime-valid interval (supported only when it clears anchor.minImprovement; contradicted when wholly below zero), pin: { step, best_step, tied_steps, supported, reason }, warnings?: string[] (e.g. a later step superseded a check whose final part never landed), point: { step, at, n, reward_rate, anchor_rate, anchor_ci: [lo, hi], anchor_corrected, masked, gap_se }, underpowered: string|null, history: [AnchorPoint] (last 20), items: [{ request_id, reward_grade, anchor_verdict, kind: 'seam'|'false_fail'|'agree'|'masked' }], counts: { seam, false_fail, agree, masked }, spend_micros }. 409 code=held when already held; 402 budget; 422 no anchor.",
  ...
  },
  {
    ...
    "notes": "A fixed-sample 95% interval, not the anytime-valid one: nothing was selected on this data. Allowed on a HELD session — a hold is exactly when you want this number. 422 when a prompt was used in a check or the session has no anchor; 402 when the session budget is spent."
  },
  {
